...
export default async function PublicAuthorPage({ params }: { params: Promise<{ username: string; author: string }> }) {
  ...
  const booksRes = await supabase
    .from("user_books")
    .select(
      "id,library_id,visibility,title_override,authors_override,cover_original_url,cover_crop,edition:editions(isbn13,title,authors,cover_url,subjects),media:user_book_media(kind,storage_path)"
    )
