...
export default async function PublicSubjectPage({ params }: { params: Promise<{ username: string; subject: string }> }) {
  ...
  const subjectName = safeDecode(subject ?? "").trim();
  ...
  if (usernameNorm && usernameNorm !== username) {
    permanentRedirect(`/u/${usernameNorm}/s/${encodeURIComponent(subjectName)}`);
  }
  ...
  if (alias && alias !== usernameNorm) {
    permanentRedirect(`/u/${alias}/s/${encodeURIComponent(subjectName)}`);
  }
  ...
  return (
    <main className="container">
      <div className="card">
        ...
        <div style={{ marginTop: 8 }}>{subjectName || "—"}</div>
      </div>
      ...
      <div className="row muted" style={{ marginTop: 14, justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
        ...
        <div className="row muted" style={{ gap: 8 }}>
          <span>Subject</span>
          <span style={{ color: "var(--fg)" }}>{subjectName || "—"}</span>
