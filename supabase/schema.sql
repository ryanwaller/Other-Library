...
as $$
  select
    (auth.uid() = target)
    or (exists (select 1 from public.profiles p where p.id = target and p.visibility = 'public'))
...
as $$
  select
    ...
    or (
      public.can_view_profile(book.owner_id)
      and (
        book.visibility = 'public'
        or (
          book.visibility in ('inherit', 'followers_only')
          and (
            exists (select 1 from public.profiles p where p.id = book.owner_id and p.visibility = 'public')
            or public.is_approved_follower(auth.uid(), book.owner_id)
          )
        )
      )
    );
